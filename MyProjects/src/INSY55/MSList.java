...
public class MSList {
	...
	public void updateTable()
	{
		...
		if (conn != null)
		{
			String sql="Select mstud_no,lrn,lname,fname,mname,gsec from mlist";
		...
		try
		{
			...
			while(rs.next())
			{
				columnData[0] = rs.getString("mstud_no");
	...
	private void initialize() {
		...
		JLabel lblNewLabel_3 = new JLabel("MODULAR STUDENTS");
		...
		tblmlist = new JTable();
		tblmlist.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) {
				int i = tblmlist.getSelectedRow();
				DefaultTableModel model = (DefaultTableModel)tblmlist.getModel();
		...
		tblmlist.setFont(new Font("Tahoma", Font.PLAIN, 12));
		scrollPane.setViewportView(tblmlist);
		tblmlist.setBorder(new BevelBorder(BevelBorder.LOWERED, null, null, null, null));
		tblmlist.setBackground(SystemColor.menu);
		...
		txtSearch.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				DefaultTableModel table = (DefaultTableModel)tblmlist.getModel();
				String search = txtSearch.getText().toUpperCase();
				TableRowSorter<DefaultTableModel> tr = new TableRowSorter<DefaultTableModel>(table);
				tblmlist.setRowSorter(tr);
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(txtLRN.getText().equals("") || txtFName.getText().equals("") || txtLName.getText().equals("") || comboBox_gsec.getSelectedItem().toString().equals(null))
				{
					...
					String sql=	"insert into mlist (lrn,lname,fname,mname,gsec)values(?,?,?,?,?)";
					try {
							pst = conn.prepareStatement(sql);
		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tblmlist.getSelectedRow() == -1) {
					JOptionPane.showMessageDialog(null, "Please select a row to edit!");
				}
				else {
					try {
					int row = tblmlist.getSelectedRow();
					String value = tblmlist.getModel().getValueAt(row, 0).toString();
					String sql = "update mlist set lrn=?, lname=?, fname=?, mname=?, gsec=? where mstud_no =" +value;
		...
		btnDelete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(tblmlist.getSelectedRow() == -1) {
					...
					int row = tblmlist.getSelectedRow();
					String cell = tblmlist.getModel().getValueAt(row, 0).toString();
					String sql = "delete from mlist where mstud_no =" +cell;
					try {
						pst=conn.prepareStatement(sql);
						pst.execute();
						DefaultTableModel model = (DefaultTableModel)tblmlist.getModel();
